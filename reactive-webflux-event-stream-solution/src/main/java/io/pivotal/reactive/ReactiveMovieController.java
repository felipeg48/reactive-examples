...
public class ReactiveMovieController {
	...
	String[] nameArray = "sang,paul,james,mark,alan,kevin,bill".split(",");

	@GetMapping(value = "/randomNames", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
	public Flux<String> getRandomNames() {
		return Flux	.fromStream(Stream.generate(() -> nameArray[new Random().nextInt(nameArray.length)]))
					.take(20);
	}

	// DONE-reactive-webflux-event-stream-04: Do curl
	...
	@GetMapping(value = "/randomNamesWithNumbers", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
	public Flux<Tuple2<Long, String>> getRandomNamesWithNumbers() {
		Flux<Long> intervals = Flux.interval(Duration.ofSeconds(1));
		Flux<String> names = Flux	.fromStream(Stream.generate(() -> nameArray[new Random().nextInt(nameArray.length)]))
									.take(20);
		return Flux	.zip(intervals, names);
	}

	// DONE-reactive-webflux-event-stream-06: 
	// Do curl -i http://localhost:8090/movies/2/movieevents
	...
	//  it returns NameEvent as a TEXT_EVENT_STREAM_VALUE, something like
	//  data:{"name":"paul","date":1512180552417}
    //  data:{"name":"bill","date":1512180553423}
    //  data:{"name":"bill","date":1512180554417}
	//  ...
	@GetMapping(value = "/nameevents", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
	public Flux<NameEvent> getNameEvents() {
			Flux<Long> intervals = Flux	.interval(Duration.ofSeconds(1))
										.take(TEN);
			Flux<NameEvent> nameEvents = Flux.fromStream(Stream.generate(() -> new NameEvent(nameArray[new Random().nextInt(nameArray.length)], new Date())));
			return Flux	.zip(intervals, nameEvents)
						.map(Tuple2<Long, NameEvent>::getT2);
	}

}

// DONE-reactive-webflux-event-stream-10: Take a look at the TO DO list in the
